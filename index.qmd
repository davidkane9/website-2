...
ggplot(top_songs, aes(x = week_num, y = rank, color = song_artist)) +
  ...
  theme(
    ...
    legend.key.width = unit(2.5, "cm"),  # Ensure sufficient width
    legend.text = element_text(size = 9),  # Compact text size
    legend.margin = margin(t = 0, r = 20, b = 0, l = 0),  # Right margin to shift content
    panel.grid.minor = element_blank(),
    panel.grid.major = element_line(color = "grey90"),
    plot.background = element_rect(fill = "#F5F5F5", color = NA),
    plot.margin = unit(c(0.5, 1.5, 0.5, 0.5), "cm")  # Extra right margin
  ) +
  guides(color = guide_legend(nrow = 3, byrow = TRUE))  # Explicitly set to 3 rows

# Optional: Save with increased width to ensure visibility
